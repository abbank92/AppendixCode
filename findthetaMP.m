function thetanew = findthetaMP(A, mE, epsln, r, thetawant, iter, radtol, ...
     smalltol, h)
% Alex Bank MP update (January 17 2020)
...
ind = find((abs(eM) < (mp('1') + radtol)) & (abs(eM) > (mp('1') - radtol)));
eM = eM(ind);
...
if (isempty(eM)) % check if M has an eigenvalue with magnitude 1
   ...
   for j = 1: length(theta)
	   if (theta(j) < 0)
			theta(j) = theta(j) + mp('2*pi');
	   end

       Ashift = A - (r*(cos(theta(j)) + mp('1i')*sin(theta(j))))*eye(mp(n));
       s = svd(Ashift);
   ...
   for j=1:length(theta)     
      ...
      inttol = mp('.01') * (thetahigh - thetalow);

	  % this is needed for the last interval
	  if (thetawant+mp('2*pi') > thetalow + inttol & ...
		  thetawant+mp('2*pi') < thetahigh - inttol)
		  thetawantt = thetawant + mp('2*pi');
	  else
      ...
      if thetawantt > thetalow + inttol & ...
         ...
         % lower midpoint
		 thetamid = (thetalow + thetawantt)/mp('2');
         
         
		 % shift thetamid into the interval [-pi,pi] again
		 if (thetamid >= mp('2*pi'))             
			thetamid = thetamid - mp('2*pi');
		 end         
		 if (thetamid >= pi)
			thetamid = thetamid - mp('2*pi');
		 end
		...
		if (min(svd(A-r*exp(mp('1i')*thetamid)*eye(mp(n)))) <= epsln)
			ind = ind + 1;
         ...
         % upper midpoint
         thetamid = (thetawantt + thetahigh)/mp('2');
         ...
         thetamid = (thetalow + thetahigh)/mp('2');    
  ...
  if isreal(A)
     indx = find(thetanew >= 0 | thetanew == mp('-pi'));
     ynew = thetanew(indx);
  end
