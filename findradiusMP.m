function [f, z] = findradiusMP(A, epsln)
...
smalltol = mp('1e-18')*max(norm(A),epsln);
...
if epsln == 0  % pseudospectrum is just the spectrum
   ...
   while ~no_thetaeig & r > rold    % r is increasing in exact arithmetic
      ...
      [r, thetabest] = radiushelperMP(A, mE, theta, realtol, rold, iter,h);
      ...
      if r > rold
        ...
        thetaold = theta;
      	theta = findthetaMP(A, mE, epsln, r, thetabest, iter, radtol, ...
          	 smalltol, h);
        if (n >= 30)
   ...
   f = r;
   z = r*(cos(thetabest) + mp('1i')*sin(thetabest));
   
   if isreal(A) & ~isreal(z)
       z = [z; r*(cos(thetabest) - mp('1i')*sin(thetabest))];
   end
